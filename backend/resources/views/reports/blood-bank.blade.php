...
<table class="brand"><tr><td width="76%"><strong>مشفى محمد بن زايد الإماراتي</strong><h1>{{ $metadata['title'] }}</h1>{{ $moduleLabel ?? 'بنك الدم · تقرير سجلات' }}</td><td width="24%"><img src="var:hospitalLogo" width="132" alt="هوية المشفى"></td></tr></table>
...
@php($scale = isset($printWidth) ? $printWidth / $totalWidth : ($detail ? 186 / $totalWidth : 1))
...
@endforeach
<p class="note">{{ $reportNote ?? 'يعرض هذا التقرير السجلات المحفوظة؛ لا يمثل اعتمادًا طبيًا أو شهادة أهلية للتبرع.' }}</p>
</body></html>
