...
class DossierWizardQueries
{
    public function snapshot(array $f, int $id): array
    {
        return DB::transaction(function () use ($f, $id) {
            ...
            $progress = DB::table('dossier_section_progress')->where('dossier_id', $id)->orderBy('id')->get()->keyBy('section');
            $visitId = $progress->get('visit')?->visit_id ?? DB::table('visits')->where('dossier_id', $id)->where('facility_id', $f['id'])->where('status', 'draft')->whereNull('voided_at')->orderBy('visit_date')->orderBy('id')->value('id');
            ...
            return ['id' => $id, 'code' => $d['code'], 'status' => $d['status'], 'lock_version' => $d['lock_version'], 'opening_date' => $d['opening_date'], 'patient' => $p,
