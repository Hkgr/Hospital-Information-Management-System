...
class DossierReports
{
    ...
    public function document(Request $r, array $f, array $filters, ?int $dossier = null, ?int $visit = null): array
    {
        ...
        [$sections,$identity,$draft] = DB::transaction(function () use ($queries, $f, $filters, $dossier, $visit) {
            ...
            $q = $queries->actualVisits($f)->where('v.dossier_id', $dossier);
            if ($visit) {
                $q->where('v.id', $visit);
            }
            ...
            $rx = DB::table('visit_prescriptions as p')->join('visit_prescription_items as i', 'i.prescription_id', '=', 'p.id')->join('clinics as c', 'c.id', '=', 'p.prescribing_clinic_id')->join('staff as s', 's.id', '=', 'p.prescribing_staff_id')->where('p.facility_id', $f['id'])->whereIn('p.visit_id', $ids)->whereNull('p.voided_at')->whereNull('i.voided_at')->orderBy('p.visit_id')->orderBy('i.display_order')->orderBy('i.id')->limit(config('dossiers.report_detail_limit') + 1)->get(['i.id', 'p.visit_id', 'p.prescribed_on', 'p.note as general_note', 'i.note', 'i.medication_code_snapshot', 'i.medication_name_snapshot', 'c.name_ar as clinic', 's.full_name as doctor']);
            $total += $rx->count();
            $this->limit($total, config('dossiers.report_detail_limit'));
            $sections[] = $this->section('الأدوية الموصوفة', $common, $rx->map(fn ($e) => ['id' => $e->id, 'code' => $byId[$e->visit_id]->visit_no, 'date' => $e->prescribed_on, 'name' => $e->medication_code_snapshot.' · '.$e->medication_name_snapshot, 'clinic' => $e->clinic, 'doctor' => $e->doctor, 'note' => implode("\n", array_filter([$e->general_note, $e->note]))])->all(), 'وصفة فقط؛ لا تمثل صرفًا أو إعطاءً. تعريف الدواء محفوظ وقت إدراجه.', ['date' => 'date']);
            foreach (['dispensed' => 'الأدوية المصروفة', 'administered' => 'الأدوية المعطاة'] as $kind => $title) {
                $q = $kind === 'dispensed' ? DB::table('visit_medications as e')->select('e.*', 'e.dispensed_on as date') : DB::table('dose_session_items as e')->join('dose_sessions as s', 's.id', '=', 'e.dose_session_id')->select('e.*', 's.visit_id', 's.administered_on as date')->whereNull('s.voided_at');
                $alias = $kind === 'dispensed' ? 'e' : 's';
                $rows = $q->where($alias.'.facility_id', $f['id'])->whereIn($alias.'.visit_id', $ids)->when($kind === 'dispensed', fn ($q) => $q->whereNull('e.voided_at'))->orderBy('e.id')->limit(config('dossiers.report_detail_limit') + 1)->get();
                $total += $rows->count();
                $this->limit($total, config('dossiers.report_detail_limit'));
                $sections[] = $this->section($title, ['code' => 'كود الزيارة', 'date' => 'التاريخ', 'name' => 'اسم الدواء المحفوظ', 'note' => 'الجرعة والكمية'], $rows->map(fn ($e) => ['id' => $e->id, 'code' => $byId[$e->visit_id]->visit_no, 'date' => $e->date, 'name' => $e->medication_name_snapshot, 'note' => trim(($e->dose_text ?? '').' · '.$e->quantity.' '.$e->quantity_unit)])->all(), 'سجل تاريخي مستقل عن الوصفة؛ لا ينشئ المعالج صرفًا أو إعطاءً.', ['date' => 'date']);
            }
        ...
        $meta['title'] = ($visit ? 'تقرير زيارة' : ($dossier ? 'تقرير إضبارة' : 'قائمة الإضبارات')).($draft ? ' — مسودة — غير مكتملة' : '');
