    ...
    public function read(array $f, int $visit): array
    {
        ...
        foreach (['services' => ['service_id', 'performed_by'], 'procedures' => ['procedure_id', 'specialist_id']] as $kind => [$catalog,$doctor]) {
            $data[$kind] = DB::table('visit_'.$kind.' as e')->join($kind.' as n', 'n.id', '=', 'e.'.$catalog)->leftJoin('clinics as c', 'c.id', '=', 'e.clinic_id')->leftJoin('staff as s', 's.id', '=', 'e.'.$doctor)
                ...
                ->get(['e.id', 'e.lock_version', "e.$catalog as catalog_id", 'n.code', 'n.name_ar', 'e.clinic_id', 'c.name_ar as clinic_name', "e.$doctor as doctor_id", 's.full_name as doctor_name', 'e.note', 'e.performed_on', 'e.quantity', ...($kind === 'services' ? ['e.status'] : [])])->all();
        }
        $rx = DB::table('visit_prescriptions as p')->join('clinics as c', 'c.id', '=', 'p.prescribing_clinic_id')->join('staff as s', 's.id', '=', 'p.prescribing_staff_id')->where('p.facility_id', $f['id'])->where('p.visit_id', $visit)->whereNull('p.voided_at')->first(['p.id', 'p.lock_version', 'p.prescribing_clinic_id', 'p.prescribing_staff_id', 'p.prescribed_on', 'p.note', 'c.name_ar as clinic_name', 's.full_name as doctor_name']);
        $data['prescription'] = $rx ? (array) $rx + ['items' => DB::table('visit_prescription_items')->where('prescription_id', $rx->id)->where('facility_id', $f['id'])->whereNull('voided_at')->orderBy('display_order')->orderBy('id')->get(['id', 'medication_id', 'medication_code_snapshot as code', 'medication_name_snapshot as name_ar', 'note', 'display_order', 'lock_version'])->all()] : null;
