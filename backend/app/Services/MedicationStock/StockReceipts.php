...
class StockReceipts
{
    ...
    public function listing(array $facility, array $filters): array
    {
        $query = DB::table('medication_receipts as r')->where('r.facility_id', $facility['id'])
            ...
            ->leftJoin('funding_sources as f', 'f.id', '=', 'r.funding_source_id')
            ->select('r.*', 'st.name_ar as store_name_ar', 'su.name_ar as supplier_name_ar', 'f.name_ar as funding_name_ar');
        ...
        $write = function () use ($request, $facility, $input, $id) {
            try {
                return DB::transaction(function () use ($request, $facility, $input, $id) {
                    ...
                    $this->requireFunding((int) $input['funding_source_id']);
                    $fields = Arr::only($input, ['store_id', 'receipt_no', 'supplier_id', 'funding_source_id', 'received_on', 'invoice_number', 'note']);
                    ...
                    app(ClinicAudit::class)->record($request, $facility['id'], $id, $old ? 'updated' : 'created', $old, Arr::only($this->find($facility, $id), ['receipt_no', 'status', 'store_id', 'supplier_id', 'funding_source_id', 'received_on']), 'medication_receipt');
    ...
    public function confirm(Request $request, array $facility, int $id, int $version): int
    {
        return DB::transaction(function () use ($request, $facility, $id, $version) {
            ...
            foreach ($items as $item) {
                ...
                if ($batch) {
                    if ((int) $batch->funding_source_id !== (int) $receipt['funding_source_id']) {
                    ...
                    $batchId = DB::table('medication_batches')->insertGetId([
                        ...
                        'funding_source_id' => $receipt['funding_source_id'], 'unit_cost' => $item->unit_cost,
    ...
    private function requireFunding(int $id): void
    {
        if (! DB::table('funding_sources')->where('id', $id)->where('is_active', true)->exists()) {
            throw ValidationException::withMessages(['funding_source_id' => 'اختر جهة تمويل فعالة.']);
        }
    }
    ...
    private function row(object $row, bool $detail, $items = null): array
    {
        $data = [
            ...
            'funding_source_id' => (int) $row->funding_source_id, 'funding_name_ar' => $row->funding_name_ar,
