...
class DoctorReports
{
    ...
    public function export(Request $request, array $facility, array $filters, string $format, ?int $id = null): Response
    {
        $rows = DB::transaction(function () use ($facility, $filters, $id) {
            ...
            $patients = $this->counts->roster($facility, $raw->pluck('id')->all())->limit(config('clinics.export_patient_limit') + 1)->get();
            if ($patients->count() > config('clinics.export_patient_limit')) {
                throw new DoctorException('EXPORT_LIMIT_EXCEEDED', 'عدد المرضى أكبر من الحد الآمن للتقرير. ضيّق النطاق.', 422);
            }
            $byPatients = $patients->groupBy('owner_id');

            return array_map(function ($row) use ($grouped, $byPatients) {
        ...
        $response = $this->renderer->response(['rows' => $rows, 'columns' => $columns, 'labels' => self::COLUMNS, 'metadata' => $metadata, 'detail' => $id !== null, 'linkTitle' => 'العيادات الحالية في المنشأة', 'patientTitle' => DirectoryPatientTable::TITLE], $format);
