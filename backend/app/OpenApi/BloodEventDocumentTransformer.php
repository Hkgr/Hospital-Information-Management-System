...
class BloodEventDocumentTransformer extends ClinicDocumentTransformer
{
    public function __invoke(OpenApi $document): void
    {
        ...
        $fields += ['blood_donation_id' => $integer(), 'blood_transfusion_id' => $integer(), 'reporting_period_id' => new IntegerType, 'legacy' => new IntegerType, 'entered_by' => new IntegerType, 'updated_by' => $integer(), 'created_at' => $nullable(), 'updated_at' => $nullable()];
        ...
        foreach ($document->paths as $path) {
            ...
            foreach ($path->operations as $op) {
                ...
                $op->addResponse(Response::make(422)->setDescription('Invalid quantity, fields, stage link, relationship, reporting period or export limit.')->setContent('application/json', Schema::fromType($report ? new AnyOf([$validation, $this->object(['error' => $this->object(['code' => (new StringType)->enum(['EXPORT_LIMIT_EXCEEDED']), 'message' => $text()])])]) : $validation)));
            }
        }
    }
}
