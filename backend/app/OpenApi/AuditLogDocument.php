...
class AuditLogDocument extends ClinicDocumentTransformer
{
    public function __invoke(OpenApi $document): void
    {
        foreach ($document->paths as $path) {
            ...
            if ($route !== 'audit' && $route !== 'audit/{id}') {
                continue;
            }
            foreach ($path->operations as $op) {
                $op->security = [new SecurityRequirement(['bearerAuth' => []])];
                $this->operation($op, $route === 'audit/{id}');
            }
        }
    }

    public function operation(Operation $op, bool $detail = false): void
    {
        $s = fn () => new StringType;
        ...
        foreach ([401 => 'Unauthenticated', 403 => 'FACILITY_ACCESS_DENIED; no entries or totals', 404 => 'AUDIT_NOT_FOUND; no internals', 422 => 'Invalid filters', 500 => 'AUDIT_UNAVAILABLE; no internal details'] as $status => $description) {
