    ...
    private function request(Operation $op, string $route): void
    {
        ...
        if (str_ends_with($route, '/void')) {
            ...
            if (str_contains($route, '/doses/')) {
                $fields += ['session_resolution' => $s()->enum(['scheduled', 'cancelled']), 'planned_on' => $date()->setDescription('Required whenever session_resolution=scheduled, including carry-forward. Never inferred from the old date.'), 'carry_forward' => (new BooleanType)->setDescription('Only an obsolete session with explicit scheduled resolution and planned_on may be carried forward.'), 'session_lock_version' => $i(), 'plan_lock_version' => $i()];
            ...
            $fields += ['status' => $s()->enum(['scheduled', 'cancelled']), 'reason' => $s(), 'planned_on' => $date()->setDescription('Required whenever status=scheduled, including carry-forward. Never inferred from the old date.'), 'carry_forward' => (new BooleanType)->setDescription('Only an obsolete session with status=scheduled and explicit planned_on may be carried forward.'), 'plan_lock_version' => $i()];
