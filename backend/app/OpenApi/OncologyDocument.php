    ...
    private function request(Operation $op, string $route): void
    {
        ...
        if (str_ends_with($route, '/void')) {
            ...
            if (str_contains($route, '/doses/')) {
                $fields += ['session_resolution' => $s()->enum(['rescheduled', 'missed', 'cancelled', 'referred']), 'planned_on' => $date(), 'carry_forward' => new BooleanType, 'session_lock_version' => $i(), 'plan_lock_version' => $i()];
            ...
            $fields += ['status' => $s()->enum(['rescheduled', 'missed', 'cancelled', 'referred']), 'reason' => $s(), 'planned_on' => $date(), 'carry_forward' => new BooleanType, 'plan_lock_version' => $i()];
