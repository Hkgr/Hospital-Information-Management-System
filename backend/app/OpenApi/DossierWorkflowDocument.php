    ...
    public function operation(Operation $op, string $route): void
    {
        ...
        if ($write) {
            ...
            if ($route === 'dossiers/diagnoses') {
                ...
                $fields += $personal + ($op->method === 'post' ? ['person_mode' => (new StringType)->enum(['existing', 'new']), 'patient_id' => $int(), 'visit_date' => $text(), 'visit_type_id' => $int()] : ['patient_lock_version' => $int()]);
            }
            ...
            $type->required = array_merge($type->required, $route === 'dossiers/diagnoses' ? ['code', 'name_ar'] : (str_ends_with($route, '/medical') ? ['is_oncology'] : (str_contains($route, '/visits') ? ['visit_date', 'visit_type_id', 'is_referred', 'diagnoses'] : ['opening_date', ...($op->method === 'post' ? ['person_mode', 'visit_date', 'visit_type_id'] : ['code', 'patient_lock_version', 'first_name', 'family_name', 'birth_date_accuracy', 'gender', 'displacement_status'])])));
        ...
        if (in_array($route, ['dossiers/diagnoses', 'dossiers/medications'], true)) {
            ...
            $body = $this->object(['data' => $this->object(['today' => $text(), 'capabilities' => $this->object(array_fill_keys(['create', 'personal_update', 'medical_update', 'visits_create', 'visits_update', 'patients_search', 'patients_create', 'patients_update', 'diagnoses_create', 'clinical_update', 'attachments_view', 'attachments_upload', 'attachments_download', 'attachments_void', 'finalize', 'visits_complete', 'export', 'audit', 'medications_create'], new BooleanType)), 'governorates' => $this->list($this->object(['id' => $int(), 'name_ar' => $text()])), 'visit_types' => $this->list($choice)])]);
