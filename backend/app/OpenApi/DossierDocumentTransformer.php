    ...
    public function __invoke(OpenApi $document): void
    {
        ...
        foreach ($document->paths as $path) {
            ...
            foreach ($path->operations as $op) {
                ...
                $op->description = 'Patient Card read contract: patients is the one global identity; code/patient_code are the same canonical code. id is the local clinical-context route ID; card_id is the global patient ID. Other facility medical contexts and visits remain inaccessible. Requires dossiers.view in the explicit active facility, active account and Sanctum Bearer api ability. All responses private, no-store. Draft and active dossiers are discoverable; status=all (default), draft or active filters before pagination/totals. No inferred legacy links. Latest/count/history use explicitly linked draft or complete, nonvoided visits up to facility today ordered visit_date DESC, id DESC. NULL reporting periods do not exclude visits. Diagnosis dates can be unknown; diagnosis clinic is independent from visit context. Search normalizes whitespace and treats %/_ literally. Writes are separate permission-scoped section endpoints; Phase 3 adds separately authorized clinical, completion, report and attachment operations.';
