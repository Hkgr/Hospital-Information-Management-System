...
class DossierDocumentTransformer extends ClinicDocumentTransformer
{
    public function __invoke(OpenApi $document): void
    {
        ...
        $visit = $this->object(['id' => $integer(), 'visit_no' => $text(), 'visit_date' => $text(), 'visit_clinic' => $nullable(), 'attending_doctor' => $nullable(), 'diagnoses' => $this->list($diagnosis)]);
        ...
        $row = $this->object(['id' => $integer(), 'code' => $text(), 'opening_date' => $text(), 'patient_code' => $text(), 'patient_name' => $text(), 'is_oncology' => new BooleanType, 'visit_count' => $integer(), 'latest_visit_id' => (new IntegerType)->nullable(true), 'diagnoses' => $this->list($diagnosis)]);
        ...
        $detail = $this->object(['id' => $integer(), 'facility_id' => $integer(), 'code' => $text(), 'opening_date' => $text(), 'disability_text' => $nullable(), 'clinical_history' => $nullable(), 'is_oncology' => new BooleanType, 'patient' => $personal, 'oncology' => $oncology, 'visit_count' => $integer(), 'latest_visit' => $visit->clone()->nullable(true)]);
        ...
        foreach ($document->paths as $path) {
            ...
            foreach ($path->operations as $op) {
                ...
                $op->description = 'Read-only Phase 1. Requires dossiers.view in the explicit active facility, active account and Sanctum Bearer api ability. All responses private, no-store. Only explicitly linked active dossiers; no inferred legacy links. Latest/count/history use completed, nonvoided visits up to facility today ordered visit_date DESC, id DESC. Diagnosis dates can be unknown; diagnosis clinic is independent from visit context. List search normalizes whitespace and treats %/_ literally. Server pagination and full filtered total, deterministic sort. Previous visits are paginated. No creation, edit, reports or attachments in this phase.';
                $body = $route === 'dossiers' ? $this->object(['data' => $this->list($row), 'meta' => $meta, 'totals' => $this->object(['dossiers' => $integer()])])
                    : (str_ends_with($route, '/visits') ? $this->object(['data' => $this->list($this->object(['id' => $integer(), 'visit_no' => $text(), 'visit_date' => $text()])), 'meta' => $meta]) : $this->object(['data' => str_contains($route, '/visits/') ? $visit : $detail]));
