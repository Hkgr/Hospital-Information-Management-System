...
class StockDocumentTransformer extends ClinicDocumentTransformer
{
    public function __invoke(OpenApi $document): void
    {
        ...
        $receipt = $this->object(['id' => new IntegerType, 'facility_id' => new IntegerType, 'store_id' => new IntegerType, 'store_name_ar' => (new StringType)->nullable(true),
            ...
            'medication_source' => (new StringType)->enum(['ministry_of_health', 'al_rowad', 'other_organization', 'personal_expense', 'none']), 'funding_name_ar' => (new StringType)->nullable(true), 'received_on' => new StringType,
            ...
            foreach ($path->operations as $operation) {
                ...
                if ($operation->method === 'delete') {
                    ...
                    if (str_ends_with($route, '/options')) {
                        $source = $this->object(['code' => (new StringType)->enum(['ministry_of_health', 'al_rowad', 'other_organization', 'personal_expense', 'none']), 'name_ar' => new StringType]);
                        $fields = ['data' => $this->object(['stores' => $this->list($choice), 'suppliers' => $this->list($choice), 'medications' => $this->list($choice), 'medication_sources' => $this->list($source)])];
                    }
