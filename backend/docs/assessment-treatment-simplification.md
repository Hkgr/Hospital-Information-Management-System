...
- The plan dialog records modality, intent, a long protocol text, the doctor who wrote the protocol, and the treating doctor. Both doctors are chosen from a clinic. There is no protocol code, diagnosis link, date window, cycle count, or planned-medication list.
- After a plan is active, each dose is an explicit date. A separate **appointment without a plan** needs no plan, approval, period or visit. It requires an eligible clinic/doctor relationship on the selected date, and the existing `dossiers.treatment.schedule` permission.
...
`2026_09_22_000003_simplify_treatment_plan_fields.php` then drops regimen lines and the extra plan columns (protocol code, diagnosis link, date window, cycle counts, the single clinic/doctor pair, notes and amendment reason). It keeps modality, intent, a required protocol text, and two clinic/doctor pairs: the doctor who wrote the protocol and the treating doctor. Existing rows copy the old protocol name and the old doctor into those required fields once; dropped values are not archived. Rollback refuses while any plan revision exists.
