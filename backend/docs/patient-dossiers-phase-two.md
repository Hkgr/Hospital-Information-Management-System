...
- Visit classification was removed by the later [patient-visit workspace migration](patient-visit-workspace.md), including historical classification values. Diagnoses retain nullable dates, actors, optimistic versions and a void triple; `is_primary` remains false for new entries.
...
| POST `/{dossier}/visits` | actual visit_date, is_referred, conditional referral fields and diagnoses array (may be empty). At most one initial draft is created. |
