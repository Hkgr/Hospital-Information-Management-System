...
if ($mode === 'prepare') {
    ...
    if ($mode === 'verify') {
        ...
        app(OncologyWriter::class)->session($request, $access, $s->dossier_id, $s->id, ['request_id' => (string) Str::uuid(), 'lock_version' => $s->lock_version, 'plan_lock_version' => DB::table('oncology_plans')->where('id', $s->plan_id)->value('lock_version'), 'status' => 'scheduled', 'planned_on' => $v->visit_date, 'reason' => 'تصحيح تاريخ الزيارة لاختبار التزامن']);
