...
class BloodBankProfileWorkflowTest extends TestCase
{
    ...
    public function test_profiles_allow_no_screenings_and_completion_without_result(): void
    {
        ...
        foreach (['donor', 'recipient'] as $kind) {
            $input = BloodBankFixture::profile($f, $kind);
            $input['screenings'] = [];
            $row = $this->postJson('/api/blood-bank', $input)->assertCreated()->assertJsonPath('data.screenings', [])->json('data');
            unset($input['kind']);
            $input['lock_version'] = 1;
            $input['request_id'] = (string) Str::uuid();
            $input['screenings'] = [['analyte' => 'HCV', 'status' => 'complete']];
            $this->putJson('/api/blood-bank/'.$kind.'/'.$row['id'], $input)->assertOk()->assertJsonCount(1, 'data.screenings')->assertJsonPath('data.screenings.0.result', null);
            $this->assertDatabaseHas('blood_bank_screenings', [$kind.'_id' => $row['id'], 'analyte' => 'HCV', 'status' => 'complete', 'result' => null]);
        }
    }
    ...
    public function test_status_only_updates_preserve_historical_results_and_methods(): void
    {
        $f = BloodBankFixture::make();
        $this->withHeader('Authorization', 'Bearer '.$f['user']->createToken('workflow-test', ['api'])->plainTextToken);
        $input = BloodBankFixture::profile($f);
        $row = $this->postJson('/api/blood-bank', $input)->assertCreated()->json('data');
        DB::table('blood_bank_screenings')->where('donor_id', $row['id'])->where('analyte', 'HCV')->update(['status' => 'complete', 'result' => 'indeterminate', 'screening_test_id' => $f['test']]);
        unset($input['kind']);
        $input['lock_version'] = 1;
        $input['request_id'] = (string) Str::uuid();
        $input['screenings'] = [['analyte' => 'HCV', 'status' => 'pending']];
        $this->putJson('/api/blood-bank/donor/'.$row['id'], $input)->assertOk();
        $this->assertDatabaseHas('blood_bank_screenings', ['donor_id' => $row['id'], 'analyte' => 'HCV', 'status' => 'pending', 'result' => 'indeterminate', 'screening_test_id' => $f['test']]);
    }
    ...
    public function test_manual_addresses_round_trip_without_polluting_catalogs_and_mixed_paths_are_rejected(): void
    {
        ...
        foreach (['donor', 'recipient'] as $kind) {
            foreach ([['governorate_text' => 'محافظة خارجية', 'city_text' => 'مدينة خارجية'], ['governorate_id' => $gov, 'city_text' => 'مدينة غير مدرجة'], ['governorate_id' => $gov, 'city_id' => $city]] as $address) {
                $input = $address + BloodBankFixture::profile($f, $kind);
                $row = $this->postJson('/api/blood-bank', $input)->assertCreated()->json('data');
                $read = $this->getJson('/api/blood-bank/'.$kind.'/'.$row['id'].'?facility_id='.$f['facility'])->assertOk()->json('data');
                foreach ($address as $key => $value) {
                    $this->assertEquals($value, $read['person'][$key]);
                }
                $this->assertDatabaseHas('blood_'.$kind.'s', ['id' => $row['id']] + $address);
            }
            foreach ([['governorate_id' => $other, 'city_id' => $city], ['governorate_id' => $gov, 'governorate_text' => 'خلط'], ['governorate_text' => 'خارجي', 'city_id' => $city], ['governorate_id' => $gov, 'city_id' => $city, 'city_text' => 'خلط'], ['city_text' => 'بلا محافظة']] as $address) {
                $this->postJson('/api/blood-bank', $address + BloodBankFixture::profile($f, $kind))->assertUnprocessable();
            }
        }
        ...
        foreach ($options['blood_components'] as $component) {
            $input = ['blood_component_id' => $component['id']] + BloodBankFixture::profile($f);
            $this->postJson('/api/blood-bank', $input)->assertCreated()->assertJsonPath('data.blood_component_id', $component['id']);
        }
    }
    ...
    public function test_optional_analytes_are_unique_and_omission_does_not_delete_saved_rows(): void
    {
        ...
        $this->postJson('/api/blood-bank', $input)->assertUnprocessable();
        $input['screenings'][1]['analyte'] = 'HIV';
        $row = $this->postJson('/api/blood-bank', $input)->assertCreated()->assertJsonCount(2, 'data.screenings')->json('data');
        unset($input['kind'], $input['screenings']);
        $input['lock_version'] = 1;
        $input['request_id'] = (string) Str::uuid();
        $this->putJson('/api/blood-bank/donor/'.$row['id'], $input)->assertOk()->assertJsonCount(2, 'data.screenings');
    }
