...
type Props = { user: User; children: React.ReactNode; onLogout: () => Promise<void>; logoutPending: boolean; logoutError: string };

export default function AppShell({ user, children, onLogout, logoutPending, logoutError }: Props) {
  ...
  return <div className={styles.shell} data-collapsed={collapsed}>
    ...
    <Sidebar pathname={pathname} collapsed={collapsed} onToggle={() => setCollapsedOverride(!collapsed)} />
    <MobileSidebar open={mobileOpen} onClose={closeMobile} pathname={pathname} />
