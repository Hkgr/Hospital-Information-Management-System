...
export default function MobileSidebar({ open, onClose, pathname, granted }: { open: boolean; onClose: () => void; pathname: string; granted: string[] }) {
  ...
  return <dialog ref={dialogRef} className={styles.mobileSidebar} aria-label="قائمة التنقل" onClose={onClose}
    ...
    <div className={styles.mobileSidebarContent}><SidebarContent pathname={pathname} granted={granted} onNavigate={onClose} mobile /><FrameOrnaments /></div>
  </dialog>;
}
