...
type Props = { pathname: string; canViewClinics: boolean; canViewDoctors: boolean; canViewCatalog: boolean; canViewBloodBank?: boolean; canViewDossiers?: boolean; collapsed?: boolean; onToggle?: () => void; onNavigate?: () => void; mobile?: boolean };

export function SidebarContent({ pathname, canViewClinics, canViewDoctors, canViewCatalog, canViewDossiers = false, canViewBloodBank = false, collapsed = false, onToggle, onNavigate, mobile = false }: Props) {
  return <>
    ...
    <nav className={styles.navigation} id={mobile ? "mobile-navigation" : "desktop-navigation"} aria-label="التنقل الرئيسي">
      <NavigationItems items={primaryNavigation.filter(item => !item.permission || ({ "doctors.view": canViewDoctors, "clinics.view": canViewClinics, "catalog.view": canViewCatalog, "dossiers.view": canViewDossiers, "blood_bank.view": canViewBloodBank }[item.permission]))} pathname={pathname} onNavigate={onNavigate} />
    </nav>
