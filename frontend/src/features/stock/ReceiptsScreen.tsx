...
function ReceiptEditor({ facilityId, options, onClose, onSaved }: { facilityId: number; options: Options; onClose: () => void; onSaved: (row: Receipt) => void }) {
  ...
  const [fundingId, setFundingId] = useState(String(options.funding_sources[0]?.id ?? ""));
  ...
  async function save(event: React.FormEvent) {
    ...
    const payload = { facility_id: facilityId, store_id: Number(storeId), receipt_no: receiptNo, supplier_id: supplierId ? Number(supplierId) : null, funding_source_id: Number(fundingId), received_on: receivedOn,
  ...
  return <Modal title="إذن استلام جديد" onClose={onClose}>
    <form className={styles.form} onSubmit={save}>
      ...
      <label>جهة التمويل<select value={fundingId} onChange={e => setFundingId(e.target.value)} required>{options.funding_sources.map(row => <option key={row.id} value={row.id}>{row.name_ar}</option>)}</select></label>
