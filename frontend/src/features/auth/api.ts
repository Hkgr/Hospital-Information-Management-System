...
export async function apiRequest<T>(endpoint: string, options: RequestInit = {}, mode: "data" | "envelope" | "blob" = "data"): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const clinicMessages: Record<string, string> = {
      ...
      DOCTOR_STATE_CONFLICT: "لا تناسب العملية حالة الطبيب الحالية. أعد تحميله؛ استعادة المؤرشف تسبق إعادة تفعيله.",
      ...
      CLINIC_REFERENCED: "للعيادة ارتباطات أو تاريخ محفوظ. أعد معاينة الأثر واختر أرشفة وإزالة من الدليل. التعطيل إجراء منفصل.",
      CLINIC_STATE_CONFLICT: "لا تناسب العملية حالة العيادة الحالية. أعد تحميلها؛ استعادة المؤرشف تسبق إعادة تفعيله.",
      CLINIC_NOT_FOUND: "العيادة غير موجودة في المنشأة المحددة.",
      AUDIT_NOT_FOUND: "الحركة غير موجودة في سجل المنشأة المحددة.",
      AUDIT_UNAVAILABLE: "تعذّر تحميل سجل الحركة. حاول مجددًا.",
      USERS_ACCESS_DENIED: "لا يتوفر لك وصول إلى إدارة المستخدمين في هذه المنشأة.",
      USERS_NOT_FOUND: "المستخدم غير موجود في المنشأة المحددة.",
      USERS_UNAVAILABLE: "تعذّر إتمام إدارة المستخدمين. حاول مجددًا.",
      USER_SELF_DELETE: "لا يمكن حذف حساب الدخول الحالي.",
      USER_USERNAME_TAKEN: "اسم المستخدم مستخدم مسبقاً.",
      USER_EMAIL_TAKEN: "البريد الإلكتروني مستخدم مسبقاً.",
      USER_ROLE_INVALID: "الدور المحدد غير متاح.",
