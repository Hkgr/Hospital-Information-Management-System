...
export const pathologyFields: Record<string, string> = { source: "مصدر التقرير", status: "حالة التشريح المرضي", report_number: "رقم التقرير", external_organization: "اسم الجهة الخارجية", specimen_type: "نوع العينة", anatomical_site: "الموقع التشريحي", requested_on: "تاريخ الطلب", collected_on: "تاريخ جمع العينة", result_on: "تاريخ النتيجة", conclusion: "الخلاصة النهائية", note: "ملاحظات", unavailable_reason: "سبب عدم الإتاحة أو الإلغاء", procedure_event_id: "الإجراء المرتبط", clinic_id: "العيادة", doctor_id: "الطبيب" };
...
export type Pathology = { id: number; visit_id: number; visit_no: string; visit_date: string; source: "internal" | "external"; status: string; lock_version: number; report_number: string | null; conclusion: string | null; clinic_name?: string | null; doctor_name?: string | null; voided_at: string | null; void_reason: string | null; capabilities: { update: boolean; void: boolean }; attachments: PathologyFile[] } & Record<string, unknown>;
...
export function doctorRoleLabel(source: string): string {
  return source === "external" ? "الطبيب المراجع ضمن المشفى" : "الطبيب المنظم";
}

// UI draft cleanup only; Laravel independently enforces the same clinical invariants.
export function normalizePathologyDraft(draft: Record<string, string>, assessment = false): Record<string, string> {
  const next = { ...draft };
  if (assessment) {
