...
function canContinue(options: WizardOptions) { const c = options.capabilities; return (c.personal_update && c.patients_update) || c.medical_update || c.visits_create || c.visits_update; }
...
function Listing({ facility }: { facility: number }) {
  ...
  return <><header className={styles.heading}><div><h2>الإضبارات</h2><p>ملفات المرضى ومسوداتهم في المشفى</p></div><div>{options.data?.capabilities.create ? <Link className={styles.primary} href={`/dossiers/new?${q}`}><LuPlus aria-hidden="true" />إضافة إضبارة</Link> : <button type="button" className={styles.primary} disabled aria-describedby="add-dossier-note"><LuPlus aria-hidden="true" />إضافة إضبارة</button>}<p id="add-dossier-note" className={styles.hint}>{options.data?.capabilities.create ? "احفظ البيانات على مراحل، وتابع المسودة لاحقًا." : "تحتاج الإضافة صلاحية إنشاء إضبارة."}</p></div></header>
...
function Detail({ id, facility }: { id: string; facility: number }) {
  ...
  return <><DirectoryBack href={`/dossiers?${back}`}>العودة إلى الإضبارات</DirectoryBack>{record.loading && <p role="status">جارٍ تحميل الإضبارة…</p>}{record.error && <Failure error={record.error} retry={record.retry} />}{d && <>
    ...
    <div className={styles.actions}>{options.data && <>
      {d.status === "draft" && canContinue(options.data) && <Link className={styles.primary} href={`/dossiers/${id}/edit?${back}`}>استكمال الإضبارة</Link>}
      {options.data.capabilities.personal_update && options.data.capabilities.patients_update && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=0`}>تعديل البيانات الشخصية</Link>}
      {options.data.capabilities.medical_update && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=1`}>تعديل المعلومات الطبية</Link>}
      {options.data.capabilities.visits_update && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=2`}>مراجعة الزيارة الأولية المسودة</Link>}
    </>}</div>
