...
function Listing({ facility }: { facility: number }) {
  ...
  for (const key of ["oncology", "visits", "from", "to", "sort", "direction", "page", "per_page"]) { const value = params.get(key); if (value) q.set(key, value); }
...
function Detail({ id, facility }: { id: string; facility: number }) {
  ...
  const history = useClinicRequest<Page<Pick<Visit, "id" | "visit_no" | "visit_date">>>(`dossiers/${id}/visits?facility_id=${facility}&page=${params.get("visits_page") ?? 1}&per_page=10`, true);
  ...
  return <><DirectoryBack href={`/dossiers?${back}`}>العودة إلى الإضبارات</DirectoryBack>{record.loading && <p role="status">جارٍ تحميل الإضبارة…</p>}{record.error && <Failure error={record.error} retry={record.retry} />}{d && <>
    <header className={styles.heading}><div><h2>الإضبارة <bdi>{d.code}</bdi></h2><p>{d.patient.first_name} {d.patient.family_name} · تاريخ الإضبارة {d.opening_date}</p></div><span className={styles.active}>{d.visit_count} زيارة</span></header>
    ...
    <section className={styles.panel}><div className={styles.toolbar}><h2>سجل الزيارات</h2><p>حسب التاريخ الفعلي، ثم معرّف الزيارة عند تساوي التاريخ.</p></div>{history.loading && <p role="status">جارٍ تحميل الزيارات…</p>}{history.error && <Failure error={history.error} retry={history.retry} />}{history.data && <><DirectoryTable label="زيارات الإضبارة" headers={["رقم الزيارة", "التاريخ الفعلي", "الاستعراض"]}>{history.data.data.map(v => <tr key={v.id}><td><bdi>{v.visit_no}</bdi></td><td>{v.visit_date}</td><td><button className={styles.secondary} onClick={() => navigate("visit", String(v.id))}>{v.id === d.latest_visit?.id ? "استعراض آخر زيارة" : "استعراض الزيارة"}</button></td></tr>)}{!history.data.data.length && <tr><td colSpan={3}>لا توجد زيارات مسجلة للإضبارة.</td></tr>}</DirectoryTable><Pagination meta={history.data.meta} onPage={p => navigate("visits_page", String(p))} /></>}</section>
  </>}</>;
}
function ClinicalTable({ title, items }: { title: string; items: ClinicalItem[] }) { return <><h3>{title}</h3><DirectoryTable label={title} headers={["الاسم", "التاريخ", "التفاصيل"]}>{items.map(item => <tr key={item.id}><td>{item.name}</td><td>{item.date}</td><td>{item.dose_text || ""} {item.quantity != null ? `${item.quantity} ${item.quantity_unit ?? ""}` : "—"}</td></tr>)}{!items.length && <tr><td colSpan={3}>لا توجد بيانات مسجلة.</td></tr>}</DirectoryTable></>; }
function VisitSummary({ visit: v }: { visit: Visit }) { return <div className={styles.detailPanel}><h3>زيارة <bdi>{v.visit_no}</bdi> · {v.visit_date}</h3>{(v.visit_clinic || v.attending_doctor) && <p className={styles.hint}>سياق الزيارة المسجل: {v.visit_clinic ?? "عيادة غير مسجلة"} · {v.attending_doctor ?? "طبيب غير مسجل"}. لا يُنسب تلقائيًا إلى كل تشخيص.</p>}<DirectoryTable label="تشخيصات الزيارة" headers={["التشخيص", "تاريخ التشخيص", "العيادة", "الطبيب المسؤول"]}>{v.diagnoses.map(d => <tr key={d.id}><td>{d.name} <bdi>({d.code})</bdi></td><td>{d.diagnosed_on ?? "غير معروف"}</td><td>{d.clinic ?? "غير مسجلة"}</td><td>{d.doctor ?? "غير مسجل"}</td></tr>)}{!v.diagnoses.length && <tr><td colSpan={4}>لا توجد تشخيصات مسجلة.</td></tr>}</DirectoryTable><ClinicalTable title="الخدمات المسجلة" items={v.services} /><ClinicalTable title="الإجراءات المسجلة" items={v.procedures} /><ClinicalTable title="الأدوية المصروفة" items={v.medications} /><ClinicalTable title="الأدوية المعطاة ضمن جلسات" items={v.administered_medications} /><ClinicalTable title="النتائج المسجلة" items={v.outcomes} /></div>; }
