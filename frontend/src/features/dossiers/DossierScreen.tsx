...
function Listing({ facility }: { facility: number }) {
  ...
  const headings = ["م", "كود الإضبارة", "اسم المريض", "تشخيصات آخر زيارة", "عيادات التشخيصات", "الأطباء المسؤولون", "عدد الزيارات", "عدد الإجراءات", "الإجراءات"];
  return <><header className={styles.heading}><div><h2>الإضبارات</h2><p>ملفات المرضى ومسوداتهم في المشفى</p></div><div>{options.data?.creation.allowed ? <Link className={styles.primary} href={`/dossiers/new?${q}`}><LuPlus aria-hidden="true" />إضافة إضبارة</Link> : <button type="button" className={styles.primary} disabled aria-describedby="add-dossier-note"><LuPlus aria-hidden="true" />إضافة إضبارة</button>}<p id="add-dossier-note" className={styles.hint}>{options.data?.creation.allowed ? "احفظ البيانات على مراحل، وتابع المسودة لاحقًا." : options.loading ? "جارٍ التحقق من إمكانية الإضافة…" : options.error ? "تعذّر التحقق من إمكانية الإضافة؛ أعد المحاولة." : options.data?.creation.reason}</p>{options.error && <button className={styles.secondary} onClick={options.retry}>إعادة التحقق من إمكانية الإضافة</button>}</div></header>
...
function Detail({ id, facility }: { id: string; facility: number }) {
  ...
  return <><DirectoryBack href={`/dossiers?${back}`}>العودة إلى الإضبارات</DirectoryBack>{record.loading && <p role="status">جارٍ تحميل الإضبارة…</p>}{record.error && <Failure error={record.error} retry={record.retry} />}{d && <>
    ...
    <div className={styles.actions}>{<>
      {d.status === "draft" && d.workflow.resume_section !== null && <Link className={styles.primary} href={`/dossiers/${id}/edit?${back}&section=${d.workflow.resume_section}`}>استكمال الإضبارة</Link>}
      {d.workflow.personal_update && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=0`}>تعديل البيانات الشخصية</Link>}
      {d.workflow.medical_update && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=1`}>تعديل المعلومات الطبية</Link>}
      {d.workflow.visit.action && <Link className={styles.secondary} href={`/dossiers/${id}/edit?${back}&section=2`}>{d.workflow.visit.label}</Link>}
    </>}</div>
