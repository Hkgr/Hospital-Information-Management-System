...
export default function WorkspaceClinical({ facility, snapshot, caps, active, visited, revision, onChanged }: {
  facility: number; snapshot: Snapshot; caps: Record<string, boolean>; active: number; visited: number[]; revision: number; onChanged: () => void;
}) {
  ...
  return <>{[6, 7, 8].filter(i => visited.includes(i) || active === i).map(i => <section key={i} hidden={active !== i} className={layout.toolPanel}>
    ...
    {i === 6 ? <PathologyPanel facility={facility} dossier={snapshot.id} visit={visit.data} caps={caps} revision={revision} onChanged={onChanged} /> : <OncologyPanel facility={facility} dossier={snapshot.id} caps={caps} revision={revision} onChanged={onChanged} {...i === 8 ? { visit: visit.data } : {}} />}
  </section>)}</>;
}
