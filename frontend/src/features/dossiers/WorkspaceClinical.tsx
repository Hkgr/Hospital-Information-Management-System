...
export default function WorkspaceClinical({ facility, snapshot, caps, active, visited, revision, onChanged, medicationView }: {
  ...
  return <>{[6, 7, 8].filter(i => visited.includes(i) || active === i).map(i => <section key={i} hidden={active !== i} className={layout.toolPanel}>
    ...
    {i === 6 ? <PathologyPanel facility={facility} dossier={snapshot.id} visit={visit.data} caps={caps} revision={revision} onChanged={onChanged} /> : <OncologyPanel medicationView={i===8?medicationView:undefined} facility={facility} dossier={snapshot.id} caps={caps} revision={revision} onChanged={onChanged} medicationSource={typeof snapshot.medical.medication_source === "string" ? snapshot.medical.medication_source : null} {...i === 8 ? { visit: visit.data } : {}} />}
  </section>)}</>;
}
