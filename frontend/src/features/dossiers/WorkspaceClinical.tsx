...
export default function WorkspaceClinical({ facility, snapshot, caps, active, revision, onChanged, medicationView }: {
  facility: number; snapshot: Snapshot; caps: Record<string, boolean>; active: number; revision: number; onChanged: () => void; medicationView?: "unlinked"|"outside";
}) {
  ...
  const patient = useClinicRequest<Dossier>(snapshot.visit ? `dossiers/${snapshot.id}?facility_id=${facility}` : null, false, true, revision);
