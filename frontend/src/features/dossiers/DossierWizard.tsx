...
const steps = ["البيانات الشخصية", "المعلومات الطبية والورمية", "الزيارة والتشخيصات", "الخدمات والإجراءات", "أدوية الزيارة والنتيجة", "المرفقات ثم المراجعة", "التقييم والتشريح المرضي", "الخطة ومواعيد العلاج", "الإعطاء والصرف الفعلي"];
...
function WizardForm({ facility, options, initial }: { facility: number; options: WizardOptions; initial?: Snapshot }) {
  ...
  const [base, setBase] = useState(initial); const [step, setStep] = useState(() => {
    if (!initial) return 0;
    const requested = params.get("section")==="8"?"4":params.get("section");
  ...
  const [medicationPane,setMedicationPane]=useState<"prescription"|"administration"|"dispensing">(params.get("section")==="8"?"administration":"prescription");
  ...
  return <div className={layout.wizard}>
    ...
    {base?.visit&&(tools.length>0||step>5||step===4&&medicationPane!=="prescription")&&<WorkspaceClinical facility={facility} snapshot={base} caps={caps} active={step===4&&medicationPane!=="prescription"?8:step} medicationView={medicationPane==="prescription"?undefined:medicationPane} visited={tools} revision={clinicalRevision} onChanged={()=>setClinicalRevision(v=>v+1)}/>}
    </div></div>
  </div>;
}
