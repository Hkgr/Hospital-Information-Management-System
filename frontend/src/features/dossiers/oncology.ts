...
export type SessionDose = {id:number;session_id:number;given_on:string;dose_name:string;complaint:string;recommendations:string;nurse_id:number;nurse_name?:string;lock_version:number};
export type Session = Values & {has_voided_dose:boolean;doses?:SessionDose[];revision_id:number|null;current_revision_id:number|null;plan_lock_version:number|null;id:number;lock_version:number;plan_id:number|null;clinic_id:number;doctor_id:number;plan_number:string;planned_on:string;status:string;dose_id:number|null;visit_id:number|null};
