...
export type Session = Values & {has_voided_dose:boolean;revision_id:number;current_revision_id:number;plan_lock_version:number;id:number;lock_version:number;plan_id:number;clinic_id:number;doctor_id:number;plan_number:string;planned_on:string;status:string;dose_id:number|null;visit_id:number|null};
