...
import { assessmentFieldApplies, normalizePathologyDraft, assessmentFields, dispositionLabels, pathologyFields, type Assessment, type Pathology } from "./pathology";
import styles from "../clinics/clinics.module.css";
import layout from "./wizard.module.css";

type Saved = Pathology | Assessment;
export default function PathologyEditor({ facility, dossier, visit, record, assessment = false, onClose, onSaved, onRefresh }: { facility: number; dossier: number; visit: Visit; record: Saved | null; assessment?: boolean; caps: Record<string, boolean>; onClose: () => void; onSaved: () => void; onRefresh: () => void }) {
  ...
  const [draft, setDraft] = useState<Record<string, string>>(() => normalizePathologyDraft({ ...strings(record), ...!assessment ? { status: "completed" } : {}, ...!record ? assessment ? { disposition: "not_assessed" } : { source: "internal", status: "completed" } : {} }, assessment));
  ...
  function applyReview() {
    ...
    if (!assessment) next.status = "completed";
    setDraft(normalizePathologyDraft(next, assessment)); setVersion(latest.lock_version); setLatest(null); setConflict(false); setError(null);
  ...
  async function save() {
    ...
    const body = JSON.stringify({ ...values, facility_id: facility, lock_version: version, ...!assessment ? { status: "completed" } : {} });
  ...
  return <Modal title={assessment ? "التقييم التشخيصي" : record ? "تعديل تقرير التشريح المرضي" : "إضافة تقرير تشريح مرضي"} size="wide" busy={busy} onClose={onClose}><form ref={form} className={`${styles.form} ${layout.editorForm}`} noValidate onSubmit={e => { e.preventDefault(); void save(); }}>
    ...
    <fieldset disabled={busy || conflict} className={layout.diagnosis}><legend>{assessment ? "قرار الطبيب" : "بيانات التقرير"}</legend><div className={styles.fields}>
      {Object.entries(labels).filter(([key]) => !["clinic_id", "doctor_id", "procedure_event_id", "evidence_pathology_id", "status", "unavailable_reason"].includes(key)).map(([key, label]) => {
        const options = key === "source" ? { internal: "ضمن المشفى", external: "من جهة خارجية" } : key === "disposition" ? Object.fromEntries(Object.entries(dispositionLabels).filter(([k]) => !["unavailable", "cancelled"].includes(k))) : null;
