...
import { LuDownload, LuFileText, LuHospital, LuLink, LuPlus, LuSearch, LuSquarePen } from "react-icons/lu";
...
function DoctorTable({ query, visible, searching, cap, onAction, onPage, onPageSize, result }: { query: string; visible: Column[]; searching: boolean; cap: Capabilities; onAction: (action: Action, doctor: Doctor) => void; onPage: (n: number) => void; onPageSize: (value: string) => void; result: ReturnType<typeof useDirectoryRequest<Page<Doctor>>> }) {
  ...
  return <>{result.error && <p role="alert" className={styles.error}>{result.error} المعروض نتائج سابقة؛ يلزم نجاح إعادة التحميل لإتاحة التصدير. <button onClick={result.retry}>إعادة المحاولة</button></p>}<div className={styles.resultSummary}><strong>{meta.total} طبيب</strong><span role={searching || result.loading ? "status" : undefined}>{searching || result.loading ? "جارٍ تحديث النتائج…" : "التصدير يشمل جميع النتائج المطابقة"}</span></div><DirectoryTable label="جدول الأطباء" busy={searching || result.loading} headers={[...visible.map(key => columns[key]), "الإجراءات"]}>
    {data.map((doctor, index) => <tr key={doctor.id}>{visible.map(key => <td key={key}>{key === "number" ? (meta.page - 1) * meta.per_page + index + 1 : key === "code" ? <Link className={styles.code} href={`/doctors/${doctor.id}?${query}`}><bdi>{doctor.code}</bdi></Link>
      ...
      <td><DirectoryRowActions name={doctor.name} href={`/doctors/${doctor.id}?${query}`} onEdit={cap.update && !doctor.archived_at ? () => onAction("edit", doctor) : undefined} onDelete={cap.delete ? () => onAction("delete", doctor) : undefined} beforeDelete={cap.link && !cap.update && !doctor.archived_at && <button className={styles.iconButton} onClick={() => onAction("links", doctor)} aria-label={`إدارة عيادات ${doctor.name}`} title="إدارة الارتباطات"><LuLink aria-hidden="true" /></button>} editTitle="تعديل الدليل المشترك"><LifecycleActions record={doctor} name={doctor.name} canUpdate={cap.update} onAction={action => onAction(action, doctor)} /></DirectoryRowActions></td></tr>)}
