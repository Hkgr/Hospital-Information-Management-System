...
export default function DoctorPicker({ facilityId, clinicId, changes, onChange }: { facilityId: number; clinicId?: number; changes: Record<number, boolean>; onChange: (id: number, selected: boolean, original: boolean) => void }) {
    ...
    {result.error ? <p role="alert">{result.error} <button type="button" onClick={result.retry}>إعادة المحاولة</button></p>
      ...
      : <>
        ...
        <div className={styles.choices}>{result.data.data.map(doctor => <label key={doctor.id} className={styles.doctorChoice}>
          <input type="checkbox" checked={changes[doctor.id] ?? doctor.is_linked} onChange={event => onChange(doctor.id, event.target.checked, doctor.is_linked)} />
