...
import { LuHospital, LuPlus, LuSearch, LuSquarePen } from "react-icons/lu";
...
function Workspace({ facilityId, kind, id, donationId }: { facilityId: number; kind?: Kind; id?: string; donationId?: string }) {
  ...
  return <>
    {!id ? <><div className={styles.heading}><div><p className={styles.eyebrow}>بنك الدم</p><h2>المتبرعون والمستفيدون</h2><p>ملفات دائمة؛ إنشاء ملف المتبرع لا يسجّل واقعة تبرع.</p></div>{caps.create && <div className={styles.actions}>{(["donor", "recipient"] as Kind[]).map(k => <button key={k} className={styles.primary} onClick={() => setEditor({ kind: k })}><LuPlus aria-hidden="true" />إضافة {kindName(k)}</button>)}</div>}</div>
      <section className={styles.panel} aria-label="ملفات بنك الدم"><div className={styles.toolbar}><label className={styles.search}><span><LuSearch aria-hidden="true" />البحث في بنك الدم</span><input type="search" placeholder="الاسم الكامل أو كود الملف…" value={search} onChange={e => change(e.target.value)} /></label></div>
        ...
        {list.error && <RequestState error={`${list.error}${list.data ? " المعروض نتائج سابقة." : ""}`} retry={list.retry} />}
...
function ProfileDetails({ profile: p, options }: { profile: Profile; options: Options }) {
  const cities = useBloodRequest<{ id: number; name_ar: string }[]>(p.person.governorate_id ? `blood-bank/cities?facility_id=${p.facility_id}&governorate_id=${p.person.governorate_id}` : null);
  return <><section className={styles.detailPanel}><h3>البيانات الشخصية</h3>{p.patient_id && <p className={styles.hint}>مرتبطة بالمريض <bdi>{p.patient_code}</bdi>؛ تُقرأ من ملفه دون نسخ.</p>}<dl className={styles.facts}>{Object.entries(personLabels).map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{key === "governorate_id" ? options.governorates.find(g => g.id === p.person[key])?.name_ar ?? "غير مسجل" : key === "city_id" ? cities.data?.find(c => c.id === p.person[key])?.name_ar ?? "غير مسجل" : choices[key]?.[String(p.person[key])] ?? p.person[key] ?? "غير مسجل"}</dd></div>)}{p.kind === "recipient" && <div><dt>جهة المستفيد</dt><dd>{p.beneficiary_entity || "غير مسجلة"}</dd></div>}<div><dt>العيادة</dt><dd>{p.clinic_name || "غير مسجلة"}</dd></div><div><dt>الطبيب المسؤول</dt><dd>{p.doctor_name || "غير مسجل"}</dd></div><div><dt>الزمرة</dt><dd>{bloodLabel(p.blood_group, p.rh)}</dd></div><div><dt>المكوّن</dt><dd>{options.blood_components.find(c => c.id === p.blood_component_id)?.name_ar || "غير مسجل"}</dd></div></dl></section><section className={styles.panel}><div className={styles.toolbar}><h3>فحوص الملف</h3></div><DirectoryTable label="فحوص ملف بنك الدم" headers={["الفحص", "الطريقة", "الحالة", "النتيجة"]}>{p.screenings.map(s => <tr key={s.analyte}><td><bdi>{s.analyte}</bdi></td><td>{options.screening_tests.find(t => t.id === s.screening_test_id)?.name_ar || "غير محددة"}</td><td>{statuses[s.status]}</td><td>{s.result ? results[s.result] : "لا توجد نتيجة"}</td></tr>)}</DirectoryTable><p className={styles.hint}>هذه فحوص الملف؛ لا تُنسخ نتائجها إلى التبرعات ولا تدل وحدها على قبول التبرع.</p></section></>;
}
