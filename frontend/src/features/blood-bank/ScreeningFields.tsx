...
export default function ScreeningFields({ draft, profile, change, fieldError }: { draft: Record<string, string>; profile?: Pick<Profile, "screenings">; change: (key: string, value: string) => void; fieldError: (key: string) => React.ReactNode }) {
  ...
  return <div className={`${styles.full} ${layout.screenings}`}>
    ...
    {active.map((a, index) => {
      ...
      return <fieldset key={a} className={styles.picker}><legend>فحص {a}</legend><div className={styles.fields}>
        ...
        <label>حالة الفحص<select name={`screenings.${index}.status`} aria-label={`حالة ${a}`} value={draft[a]} onChange={e => change(a, e.target.value)}>{Object.entries(statuses).map(([v, name]) => <option key={v} value={v}>{name}</option>)}</select>{fieldError(`screenings.${index}.status`)}</label>
