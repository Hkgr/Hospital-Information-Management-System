...
import { type Choice, type Kind, type Options, type Profile, choices, kindName, personLabels, results, statuses, useBloodRequest } from "./api";
...
function draftOf(profile?: Profile): Draft {
  ...
  for (const a of ["HBsAg", "HCV", "HIV"]) { const screen = profile?.screenings.find(s => s.analyte === a); draft[a] = JSON.stringify({ status: screen?.status ?? "not_requested", result: screen?.result ?? null, screening_test_id: screen?.screening_test_id ?? null }); }
  return draft;
}
const labels = { ...personLabels, identity: "مصدر بيانات المستفيد", beneficiary_entity: "جهة المستفيد", blood_group: "زمرة ABO", rh: "عامل Rh", clinic_id: "العيادة", responsible_staff_id: "الطبيب المسؤول", blood_component_id: "نوع المكوّن", HBsAg: "فحص HBsAg", HCV: "فحص HCV", HIV: "فحص HIV" };
...
export default function ProfileEditor({ kind, profile, facilityId, options, onClose, onSaved, onRefresh }: { kind: Kind; profile?: Profile; facilityId: number; options: Options; onClose: () => void; onSaved: (profile: Profile) => void; onRefresh: () => void }) {
  ...
  const governorate = patientMode ? person?.governorate_id : draft.governorate_id;
  ...
  const format = (key: string, value: string) => {
    if (["HBsAg", "HCV", "HIV"].includes(key)) { const s = JSON.parse(value); return `${statuses[s.status]} · ${results[s.result] ?? "دون نتيجة"} · ${options.screening_tests.find(t => t.id === s.screening_test_id)?.name_ar ?? "الطريقة غير معروفة"}`; }
  ...
  return <><Modal title={`${base ? "تعديل" : "إضافة"} ${kindName(kind)}`} onClose={onClose} busy={busy} size="wide"><form className={styles.form} onSubmit={save}>
    ...
    <fieldset className={styles.fields} disabled={busy || conflict}>
      ...
      {patientMode ? <div className={styles.full}>{options.capabilities.patients_search && <Picker label="المريض المسجل" path={`blood-bank/patients?facility_id=${facilityId}`} selected={chosenPatient} onSelect={setChosenPatient} />}
        ...
        {person && <dl className={styles.facts}>{Object.entries(personLabels).map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{key === "governorate_id" ? options.governorates.find(g => g.id === person[key])?.name_ar ?? "غير محدد" : key === "city_id" ? cities.data?.find(c => c.id === person[key])?.name_ar ?? "غير محدد" : choices[key]?.[String(person[key])] ?? String(person[key] ?? "غير محدد")}</dd></div>)}</dl>}{fieldError("patient_id")}</div>
        : <>{Object.entries(personLabels).map(([key, label]) => <label key={key} className={key === "address_line" ? styles.full : undefined}>{label}{["first_name", "family_name"].includes(key) ? " *" : ""}
          {choices[key] ? <select aria-label={label} value={draft[key]} onChange={e => change(key, e.target.value)}>{Object.entries(choices[key]).map(([value, name]) => <option key={value} value={value}>{name}</option>)}</select>
            : ["governorate_id", "city_id"].includes(key) ? <select aria-label={label} value={draft[key]} onChange={e => { change(key, e.target.value); if (key === "governorate_id") change("city_id", ""); }}><option value="">غير محدد</option>{(key === "governorate_id" ? options.governorates : cities.data ?? []).map(o => <option key={o.id} value={o.id}>{o.name_ar}</option>)}</select>
      ...
      {clinic && <div className={styles.full}><Picker key={`${clinic.id}:${doctor?.id ?? ""}`} label="الطبيب المسؤول" path={`blood-bank/doctors?facility_id=${facilityId}&clinic_id=${clinic.id}`} selected={doctor} onSelect={value => { setDoctor(value); change("responsible_staff_id", String(value.id)); }} />{fieldError("responsible_staff_id")}{options.can_add_doctor && <button type="button" className={styles.secondary} onClick={() => setInline(true)}>إضافة طبيب</button>}</div>}
      ...
      <label className={styles.full}>نوع المكوّن<select aria-label="نوع المكوّن" value={draft.blood_component_id} onChange={e => change("blood_component_id", e.target.value)}><option value="">غير محدد</option>{options.blood_components.map(o => <option key={o.id} value={o.id}>{o.name_ar}</option>)}</select>{fieldError("blood_component_id")}</label>
      <div className={styles.full}><h3>الفحوصات</h3><p className={styles.hint}>تُنقل النتيجة من تقرير المختبر؛ لا تعني تشخيصًا أو قرار أهلية. الطريقة غير المعروفة تبقى فارغة.</p>{["HBsAg", "HCV", "HIV"].map(a => { const screen = JSON.parse(draft[a]); const set = (key: string, value: unknown) => change(a, JSON.stringify({ ...screen, [key]: value })); return <fieldset key={a} className={styles.picker}><legend>{a}</legend><label>حالة الفحص<select aria-label={`حالة ${a}`} value={screen.status} onChange={e => change(a, JSON.stringify({ ...screen, status: e.target.value, result: e.target.value === "complete" ? screen.result : null }))}>{Object.entries(statuses).map(([v, name]) => <option key={v} value={v}>{name}</option>)}</select></label><label>نوع الاختبار<select aria-label={`نوع ${a}`} value={screen.screening_test_id ?? ""} onChange={e => set("screening_test_id", e.target.value ? Number(e.target.value) : null)}><option value="">غير معروف</option>{options.screening_tests.filter(t => t.blood_bank_analyte === a).map(t => <option key={t.id} value={t.id}>{t.name_ar}</option>)}</select></label><label>النتيجة<select aria-label={`نتيجة ${a}`} disabled={screen.status !== "complete"} required={screen.status === "complete"} value={screen.result ?? ""} onChange={e => set("result", e.target.value || null)}><option value="">لا توجد نتيجة مسجلة</option>{Object.entries(results).map(([v, name]) => <option key={v} value={v}>{name}</option>)}</select></label></fieldset>; })}{fieldError("screenings")}</div>
