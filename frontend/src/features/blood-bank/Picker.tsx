...
export default function Picker({ name, label, path, choices, eventChoices = false, loading, error, retry, emptyMessage = "لا توجد خيارات مطابقة متاحة.", selected, onSelect }: { name?: string; label: string; path?: string; choices?: Choice[]; eventChoices?: boolean; loading?: boolean; error?: string; retry?: () => void; emptyMessage?: string; selected?: Choice | null; onSelect: (row: Choice) => void }) {
  ...
  return <fieldset className={`${styles.picker} ${layout.picker}`}><legend>{label}</legend>{selected && <p className={styles.hint}>المحدد: {selected.name_ar} {selected.code && <bdi>({selected.code})</bdi>}</p>}
    <input name={name} type="search" aria-label={`البحث: ${label}`} value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="الاسم أو الكود…" />
    {(path ? result.loading || search !== committed : loading) ? <p role="status">جارٍ تحميل الخيارات…</p> : failure ? <p role="alert">{failure} <button className={styles.secondary} type="button" onClick={path ? result.retry : retry}>إعادة المحاولة</button></p> : result.data?.doctor_types_configured === false ? <p role="alert">دليل أنواع الأطباء غير مهيأ. راجع مسؤول النظام لإعداد دليل الأطباء المستخدم في قسم العيادات.</p> : <><div className={`${styles.choices} ${layout.pickerChoices} ${!path && (rows?.length ?? 0) > 4 ? layout.choiceColumns : ""}`}>{rows?.map(row => <button className={styles.secondary} type="button" key={row.id} aria-pressed={row.id === selected?.id} onClick={() => onSelect(row)}><span>{row.name_ar}{row.code && <bdi>({row.code})</bdi>}</span>{row.id === selected?.id && <LuCheck aria-hidden="true" />}</button>)}</div>{!rows?.length && <p className={styles.hint}>{emptyMessage}</p>}{path && result.data && <div className={layout.pickerPagination}><Pagination meta={result.data.meta} onPage={setPage} /></div>}</>}
  </fieldset>;
}
