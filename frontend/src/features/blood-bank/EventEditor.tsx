...
import { type Choice, type Options, bloodLabel, statuses, useBloodRequest } from "./api";
...
export default function EventEditor({ kind, event, selectedPerson, facilityId, options, onClose, onSaved, onRefresh }: { kind: EventKind; event?: BloodEvent; selectedPerson?: Person; facilityId: number; options: Options; onClose: () => void; onSaved: (e: BloodEvent) => void; onRefresh: () => void }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    else payload.person = personPayload(personal, patient);
    if (kind === "benefit") {
  ...
  return <><Modal title={`${base ? "تعديل" : "تسجيل"} ${kind === "donation" ? "تبرع" : "استفادة"}`} onClose={onClose} busy={busy} size="wide"><form className={`${styles.form} ${layout.profileForm}`} onSubmit={save}>
    <p className={styles.scopeNote}>يحفظ التسجيل الشخص والواقعة معًا. لا يدل على أهلية أو قبول تبرع، ولا ينشئ مخزونًا.</p>
    ...
    <fieldset className={styles.fields} disabled={busy || conflict}>
      <div className={styles.sectionHeading}><span>01</span><h3>بيانات الشخص</h3></div>
      {!base && !selectedPerson && <label className={styles.full}>الشخص<select value={source} onChange={e => { setSource(e.target.value); change("issue_event_id", ""); }}><option value="existing">اختيار شخص موجود</option><option value="new" disabled={!options.capabilities.create}>إضافة شخص جديد مع الواقعة</option></select></label>}
      {source === "existing" ? <div className={styles.full}>{!base && !selectedPerson && <Picker label="الشخص الموجود" path={`blood-bank/people?facility_id=${facilityId}`} selected={chosen} onSelect={p => { setChosen(p); change("issue_event_id", ""); }} />}{current.loading && <p role="status">جارٍ جلب بيانات الشخص…</p>}{current.error && <p role="alert">{current.error} <button type="button" onClick={current.retry}>إعادة المحاولة</button></p>}{current.data && <><strong>{current.data.name} · <bdi>{current.data.code}</bdi></strong><p>{bloodLabel(current.data.blood_group, current.data.rh)}</p><p className={styles.hint}>البيانات الحالية للشخص مستقلة عن بيانات الدم المثبتة لهذه الواقعة.</p>{!base && <button type="button" className={styles.secondary} onClick={() => { change("blood_group", current.data!.blood_group ?? ""); change("rh", current.data!.rh ?? ""); }}>استخدام الزمرة الحالية لهذه الواقعة</button>}</>}{fieldError("person_id")}</div> : <><PersonFields draft={personal} change={(key, value) => setPersonal(p => ({ ...p, [key]: value }))} patient={patient} onPatient={setPatient} facilityId={facilityId} options={options} fieldError={fieldError} /><div className={styles.full}><p className={styles.hint}>الزمرة الحالية للشخص اختيارية؛ بيانات الواقعة تُدخل مستقلّة أدناه.</p></div><BloodFields draft={personal} change={(key, value) => setPersonal(p => ({ ...p, [key]: value }))} /></>}
      <div className={styles.sectionHeading}><span>02</span><h3>بيانات {kind === "donation" ? "التبرع" : "الاستفادة"}</h3></div>
      {kind === "benefit" && <><label className={styles.full}>نوع الاستفادة *<select required disabled={!!base} value={draft.benefit_kind} onChange={e => change("benefit_kind", e.target.value)}><option value="">اختر نوع الاستفادة</option><option value="issue">استلام/صرف مكوّن دم</option><option value="transfusion">نقل دم أُجري فعليًا للمستفيد</option></select>{fieldError("benefit_kind")}</label>
        {draft.benefit_kind === "transfusion" && <><label className={styles.full}>ارتباط عملية النقل *<select required disabled={!!base} value={draft.benefit_link_mode} onChange={e => change("benefit_link_mode", e.target.value)}><option value="">حدد ارتباط العملية</option><option value="independent">نقل مستقل؛ لا توجد واقعة صرف سابقة لهذه العملية</option><option value="linked" disabled={source !== "existing"}>استكمال صرف سابق مسجل للشخص نفسه</option></select>{fieldError("benefit_link_mode")}</label>{draft.benefit_link_mode === "linked" && <div className={styles.full}>{base ? <p>الصرف المرتبط: {base.issue_event_id}</p> : <Picker key={chosen?.id} label="واقعة الصرف السابقة" eventChoices path={chosen ? `blood-bank/events?facility_id=${facilityId}&person_id=${chosen.id}&available_issues=1` : undefined} selected={String(issueChoice?.id) === draft.issue_event_id ? issueChoice : null} onSelect={i => { setIssueChoice(i); change("issue_event_id", String(i.id)); }} />}{fieldError("issue_event_id")}<p className={styles.hint}>يظهر الصرف والنقل مرحلتين في السجل، ويُحسبان استفادة واحدة.</p></div>}</>}
      </>}
      <label>التاريخ الفعلي *<input aria-label="التاريخ الفعلي" required type="date" value={draft.occurred_on} onChange={e => change("occurred_on", e.target.value)} />{fieldError("occurred_on")}</label>
      <label>الكمية ({unitLabel(base?.quantity_unit ?? "kg")}) *<input aria-label={`الكمية (${unitLabel(base?.quantity_unit ?? "kg")})`} required type="number" inputMode="decimal" min="0.0001" max="99999999999999.9999" step="0.0001" value={draft.quantity} onChange={e => change("quantity", e.target.value)} />{fieldError("quantity")}</label>
      {base?.quantity_unit === "unit" && <p className={`${styles.hint} ${styles.full}`}>كمية تاريخية بعدد الوحدات؛ لا تُحوّل إلى كيلوغرامات.</p>}
      <BloodFields draft={draft} change={change} />{fieldError("blood_group")}
      <label className={styles.full}>نوع المكوّن *<select required value={draft.blood_component_id} onChange={e => change("blood_component_id", e.target.value)}><option value="">اختر المكوّن</option>{options.blood_components.map(c => <option key={c.id} value={c.id}>{c.name_ar}</option>)}{base?.blood_component_id && !options.blood_components.some(c => c.id === base.blood_component_id) && <option value={base.blood_component_id}>{base.component_name} (قيمة تاريخية)</option>}</select>{fieldError("blood_component_id")}</label>
      {kind === "benefit" && <><label>جهة المستفيد<input maxLength={200} value={draft.beneficiary_entity} onChange={e => change("beneficiary_entity", e.target.value)} />{fieldError("beneficiary_entity")}</label><label>عنوان الجهة<input aria-label="عنوان الجهة" maxLength={255} value={draft.entity_address} onChange={e => change("entity_address", e.target.value)} />{fieldError("entity_address")}<small>مستقل عن عنوان سكن الشخص.</small></label></>}
      <div className={styles.sectionHeading}><span>03</span><h3>العيادة والطبيب</h3></div>
      <div className={styles.full}><Picker label="العيادة" path={`blood-bank/clinics?facility_id=${facilityId}`} selected={clinic} onSelect={c => { setClinic(c); change("clinic_id", String(c.id)); if (c.id !== clinic?.id) { setDoctor(null); change("responsible_staff_id", ""); } }} />{fieldError("clinic_id")}</div>
      {clinic && <div className={styles.full}><Picker key={clinic.id} label="الطبيب المسؤول" path={`blood-bank/doctors?facility_id=${facilityId}&clinic_id=${clinic.id}`} selected={doctor} onSelect={d => { setDoctor(d); change("responsible_staff_id", String(d.id)); }} emptyMessage="لا يوجد أطباء مؤهلون مرتبطون حاليًا بالعيادة يطابقون البحث." />{fieldError("responsible_staff_id")}{options.can_add_doctor && <button type="button" className={styles.secondary} onClick={() => setInline(true)}>إضافة طبيب</button>}</div>}
      <div className={styles.sectionHeading}><span>04</span><h3>فحوص الواقعة</h3></div><ScreeningFields draft={draft} profile={base} change={change} fieldError={fieldError} />
    </fieldset>
    <div className={styles.modalActions}><button className={styles.primary} disabled={busy || conflict || !draft.clinic_id || !draft.responsible_staff_id || (source === "existing" && (!current.data || current.loading || !!current.error))}>حفظ الواقعة</button><button type="button" className={styles.secondary} disabled={busy} onClick={onClose}>إلغاء</button></div>
