...
export default function AddressFields({ draft, profile, options, cities, change, fieldError }: {
  ...
  return <>
    <fieldset className={`${styles.picker} ${styles.full}`}><legend>المحافظة</legend>
      ...
      {draft.governorate_mode === "directory" ? <><Picker label="المحافظة السورية" choices={options.governorates} selected={governorate} onSelect={g => { if (draft.governorate_id !== String(g.id)) change("city_id", ""); change("governorate_id", String(g.id)); }} />{draft.governorate_id && <button type="button" className={styles.secondary} onClick={() => { change("governorate_id", ""); change("city_id", ""); }}>إلغاء تحديد المحافظة</button>}{fieldError("governorate_id")}</>
        : <label>اسم المحافظة خارج سوريا<input aria-label="اسم المحافظة خارج سوريا" required maxLength={120} value={draft.governorate_text} onChange={e => change("governorate_text", e.target.value)} />{fieldError("governorate_text")}</label>}
    </fieldset>
    {(draft.governorate_mode === "foreign" || draft.governorate_id) && <fieldset className={`${styles.picker} ${styles.full}`}><legend>المدينة</legend>
      ...
      {draft.governorate_mode === "foreign" || draft.city_mode === "manual" ? <label>اسم المدينة<input aria-label="اسم المدينة" required={draft.city_mode === "manual"} maxLength={120} value={draft.city_text} onChange={e => change("city_text", e.target.value)} />{fieldError("city_text")}</label>
        : <><Picker key={draft.governorate_id} label="المدينة التابعة للمحافظة" choices={cities.data} loading={cities.loading} error={cities.error} retry={cities.retry} selected={city} onSelect={c => change("city_id", String(c.id))} emptyMessage="لا توجد مدينة مطابقة؛ يمكنك اختيار المدينة غير موجودة." />{draft.city_id && <button type="button" className={styles.secondary} onClick={() => change("city_id", "")}>إلغاء تحديد المدينة</button>}{fieldError("city_id")}</>}
    </fieldset>}
  </>;
}
