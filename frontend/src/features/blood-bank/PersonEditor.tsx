...
export default function PersonEditor({ person, facilityId, options, onClose, onSaved, onRefresh }: { person: Person; facilityId: number; options: Options; onClose: () => void; onSaved: () => void; onRefresh: () => void }) {
  ...
  return <Modal title="تعديل بيانات الشخص" onClose={onClose} busy={busy} size="wide"><form ref={form} onInvalidCapture={onInvalid} className={`${styles.form} ${layout.profileForm}`} onSubmit={save}><p className={styles.scopeNote}>تعديل البيانات الحالية لا يغيّر الزمرة والحقول المثبتة للوقائع السابقة.</p><PersonSummary person={base} hint="هذه بيانات الشخص الحالية؛ الوقائع السابقة تحتفظ بقيمها." />{error && <p role="alert" tabIndex={-1} className={styles.error}>{error.message} مسودتك محفوظة.</p>}{conflict && <button type="button" disabled={busy} className={styles.secondary} onClick={() => void reload()}>جلب أحدث نسخة</button>}{reloadError && <p role="alert">{reloadError}</p>}
    ...
    <fieldset className={layout.sectionList} disabled={busy || conflict}><FormSection number="01" title="بيانات الشخص" hint="الحقول المعلّمة * مطلوبة؛ تبقى هوية المريض المرتبط محفوظة."><PersonFields draft={draft} change={change} patient={base.patient_id ? { id: base.patient_id, name_ar: base.name } : null} onPatient={() => {}} base={base} facilityId={facilityId} options={options} fieldError={fieldError} /></FormSection><FormSection number="02" title="زمرة الشخص الحالية" hint="تعديل هذه القيم لا يغيّر الزمرة المثبتة لأي واقعة سابقة."><BloodFields draft={draft} change={change} fieldError={fieldError} /></FormSection></fieldset><FormActions busy={busy} disabled={busy || conflict} label="حفظ بيانات الشخص" onClose={onClose} />
  </form></Modal>;
}
