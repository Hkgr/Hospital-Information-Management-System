...
test('functional report, independent appointment, single protocol, medication panes and separate review',async()=>{
 ...
 try{
  ...
  await page.goto(url(4));await page.getByRole('tab',{name:'أدوية مصروفة من المشفى (غير مرتبطة بالجرعة)',exact:true}).click();await page.getByRole('button',{name:'إضافة وصفة',exact:true}).waitFor();await page.getByRole('tab',{name:'أدوية مرتبطة بالجرعة',exact:true}).click();await page.getByText('تمويل الوصفة مطلوب',exact:false).waitFor();await page.getByRole('tab',{name:'أدوية خارج المشفى',exact:true}).click();await page.getByText('سبب عدم تواجدها في المشفى',exact:false).waitFor();await page.getByRole('tab',{name:'النتيجة',exact:true}).click();await page.getByRole('button',{name:'تسجيل نتيجة الزيارة',exact:true}).waitFor();
