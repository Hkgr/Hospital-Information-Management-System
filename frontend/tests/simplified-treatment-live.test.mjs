...
test('functional report, independent appointment, single protocol, medication panes and separate review',async()=>{
 ...
 try{
  ...
  await page.goto(url(7));await page.getByRole('button',{name:'موعد الجرعة القادمة',exact:true}).click();const dialog=page.getByRole('dialog');
  await dialog.locator('[name="note"]').fill('مسودة موعد مستقل');const invalid=page.waitForResponse(r=>r.request().method()==='POST'&&r.url().endsWith('/treatment-sessions'));
  await dialog.getByRole('button',{name:'حفظ',exact:true}).click();assert.equal((await invalid).status(),422);await page.waitForFunction(()=>document.activeElement?.getAttribute('name')==='planned_on');assert.equal(await dialog.locator('[name="note"]').inputValue(),'مسودة موعد مستقل');await shot(page,`appointment-errors-${width}`);
  await dialog.locator('[name="planned_on"]').fill('2030-01-02');await choose(dialog,'العيادة · الطبيب الذي حدد الموعد');await choose(dialog,'الطبيب المسؤول · الطبيب الذي حدد الموعد');await shot(page,`appointment-${width}`);
  ...
  await page.getByRole('button',{name:'إضافة خطة علاجية',exact:true}).click();assert.equal(await dialog.locator('[name="protocol_code"]').count(),0);assert.equal(await dialog.getByRole('button',{name:/إضافة دواء/}).count(),0);await dialog.locator('[name="protocol_name"]').fill('اسم أو كود واحد');await shot(page,`plan-${width}`);await dialog.getByRole('button',{name:'إلغاء',exact:true}).click();
  await page.goto(url(4));await page.getByRole('tab',{name:'دواء أُعطي فعليًا',exact:true}).click();await page.getByRole('region',{name:'العلاج الفعلي للزيارة',exact:true}).waitFor();await shot(page,`medications-${width}`);await page.getByRole('tab',{name:'دواء صُرف للمريض',exact:true}).click();await page.getByText('لا توجد جلسة إعطاء محفوظة لربط صرف الدواء بها. الوصفة وحدها لا تسجل صرفًا.',{exact:true}).waitFor();await page.getByRole('tab',{name:'الوصفة ونتيجة الزيارة',exact:true}).click();
