...
test('list reuses server pagination and add cannot write; complete visual states at three widths',async()=>{
  for(const width of [390,768,1440]){const{page,context}=await pageAt(`/dossiers?facility_id=${f.facility}&per_page=10&sort=code&direction=asc`,width);try{
    ...
    await page.getByRole('link',{name:'العودة إلى الإضبارات',exact:true}).click();await page.getByRole('heading',{name:'الإضبارات',exact:true,level:2}).waitFor();assert.equal(await page.getByRole('searchbox',{name:'البحث في الإضبارات',exact:true}).inputValue(),'  أحمد    محمد  ');assert.equal(new URL(page.url()).searchParams.get('search'),'  أحمد    محمد  ');
