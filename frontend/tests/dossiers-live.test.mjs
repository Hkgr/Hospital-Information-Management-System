...
test('list reuses server pagination and add cannot write; complete visual states at three widths',async()=>{
  for(const width of [390,768,1440]){const{page,context}=await pageAt(`/dossiers?facility_id=${f.facility}&per_page=10&sort=code&direction=asc`,width);try{
    ...
    const add=page.getByRole('button',{name:'إضافة بطاقة مريض',exact:true});assert.equal(await add.isDisabled(),true);await add.evaluate(el=>el.click());assert.equal(writes.length,0);assert.equal(await page.getByRole('dialog').count(),0);
    ...
    await page.goto(`${base}/dossiers/${f.dossiers[2]}?facility_id=${f.facility}`);await page.getByText('لا توجد زيارات فعلية مرتبطة بهذه بطاقة المريض.',{exact:true}).waitFor();await capture(page,`no-visits-${width}`);
...
test('delayed actual replies, failed transport, browser history and facility changes cannot restore stale results',async()=>{
  for(const width of [390,768,1440]){const{page,context}=await pageAt(`/dossiers?facility_id=${f.facility}`,width);try{
    ...
    await page.evaluate(id=>history.pushState(null,'',`/dossiers?facility_id=${id}&search=أحمد&oncology=yes`),f.facility);await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();
    ...
    await search.fill('أحمد');await page.waitForTimeout(350);await page.evaluate(id=>history.pushState(null,'',`/dossiers?facility_id=${id}`),f.second);await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();await page.waitForTimeout(1500);assert.equal(await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).count(),0);
    await page.goto(`${base}/dossiers?facility_id=${f.other}`);await page.getByRole('heading',{name:'بطاقات المرضى غير متاحة',exact:true}).waitFor();assert.equal(await page.getByRole('region',{name:'جدول بطاقات المرضى'}).count(),0);
  }finally{await context.close();}}
