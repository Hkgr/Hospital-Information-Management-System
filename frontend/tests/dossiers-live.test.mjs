...
const gallery=new URL('../.superdesign/tmp/patient-card-regressions/dossiers-phase-one/',import.meta.url);
...
test('list reuses server pagination and add cannot write; complete visual states at three widths',async()=>{
  for(const width of [390,768,1440]){const{page,context}=await pageAt(`/patient-cards?facility_id=${f.facility}&per_page=10&sort=code&direction=asc`,width);try{
    ...
    await page.getByRole('link',{name:'بطاقة المريض',exact:true}).waitFor();
    ...
    await page.getByRole('region',{name:'جدول بطاقات المرضى',exact:true}).evaluate(el=>el.scrollLeft=-el.scrollWidth);
    ...
    await page.getByRole('searchbox',{name:'البحث في بطاقات المرضى',exact:true}).fill('  أحمد    محمد  ');await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();await page.getByRole('link',{name:`استعراض ${f.tag}-P1`,exact:true}).click();await page.getByRole('heading',{name:'الملف الورمي الحالي',exact:true}).waitFor();await page.getByRole('region',{name:'تشخيصات الزيارة',exact:true}).waitFor();await capture(page,`oncology-detail-${width}`);
    await page.getByRole('region',{name:'زيارات بطاقة المريض',exact:true}).waitFor();
    ...
    await page.getByRole('link',{name:'العودة إلى بطاقات المرضى',exact:true}).click();await page.getByRole('heading',{name:'بطاقات المرضى',exact:true,level:2}).waitFor();assert.equal(await page.getByRole('searchbox',{name:'البحث في بطاقات المرضى',exact:true}).inputValue(),'  أحمد    محمد  ');assert.equal(new URL(page.url()).searchParams.get('search'),'  أحمد    محمد  ');
    await page.getByRole('searchbox').fill('لا توجد مطابقة');await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();await capture(page,`empty-${width}`);
    await page.goto(`${base}/patient-cards/${f.dossiers[2]}?facility_id=${f.facility}`);await page.getByText('لا توجد زيارات فعلية مرتبطة ببطاقة المريض هذه.',{exact:true}).waitFor();await capture(page,`no-visits-${width}`);
  }finally{await context.close();}}
});

test('draft dossier filters and visit badges work through real saved records without writes',async()=>{
  for(const width of [390,768,1440]) {
    const {page,context}=await pageAt(`/patient-cards?facility_id=${f.facility}&sort=code&direction=asc`,width);
    try {
      await page.getByText('11 بطاقة مريض مطابقة',{exact:true}).waitFor();
      assert.equal(await page.getByRole("combobox",{name:'حالة بطاقة المريض',exact:true}).inputValue(),'all');
      const row=page.getByRole('row').filter({has:page.getByText(`${f.tag}-P2`,{exact:true})});
      assert.equal(await row.getByLabel('حالة بطاقة المريض: مسودة',{exact:true}).innerText(),'مسودة');
      await page.locator('summary').filter({hasText:'الأعمدة'}).click(); await page.getByRole('checkbox',{name:'تشخيصات آخر زيارة',exact:true}).check(); await page.locator('summary').filter({hasText:'الأعمدة'}).click(); assert.equal(await row.getByLabel('حالة الزيارة: مسودة',{exact:true}).innerText(),'مسودة');
      await page.getByRole("combobox",{name:'حالة بطاقة المريض',exact:true}).selectOption('draft');
      await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();
      assert.equal(await page.locator('tbody tr').count(),1);
      await page.getByRole('link',{name:`استعراض ${f.tag}-P2`,exact:true}).click();
      await page.getByLabel('حالة بطاقة المريض: مسودة',{exact:true}).waitFor();
      ...
      await page.getByRole('link',{name:'العودة إلى بطاقات المرضى',exact:true}).click();
      await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();
      assert.equal(await page.getByRole("combobox",{name:'حالة بطاقة المريض',exact:true}).inputValue(),'draft');
      await page.getByRole("combobox",{name:'حالة بطاقة المريض',exact:true}).selectOption('active');
      await page.getByText('10 بطاقة مريض مطابقة',{exact:true}).waitFor();
      assert.equal(await page.getByLabel('حالة بطاقة المريض: مسودة',{exact:true}).count(),0);
      await page.getByRole('link',{name:`استعراض ${f.tag}-P1`,exact:true}).click();
      await page.getByRole('heading',{name:'الملف الورمي الحالي',exact:true}).waitFor();
      await page.getByLabel('حالة بطاقة المريض: فعالة',{exact:true}).waitFor();
...
test('delayed actual replies, failed transport, browser history and facility changes cannot restore stale results',async()=>{
  for(const width of [390,768,1440]){const{page,context}=await pageAt(`/patient-cards?facility_id=${f.facility}`,width);try{
    await page.getByText('11 بطاقة مريض مطابقة',{exact:true}).waitFor();
    ...
    await search.fill('مفقود');await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();await page.waitForTimeout(1500);assert.equal(await page.locator('tbody tr').count(),0);assert.match(await page.locator('main').innerText(),/لا توجد بطاقات مرضى/);
    await search.fill('فشل');await page.getByRole('button',{name:'إعادة المحاولة',exact:true}).waitFor();await capture(page,`error-${width}`);
    await page.evaluate(id=>history.pushState(null,'',`/patient-cards?facility_id=${id}&search=أحمد&oncology=yes`),f.facility);await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();
    await page.evaluate(id=>history.pushState(null,'',`/patient-cards?facility_id=${id}&search=مفقود&visits=without`),f.facility);await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();await search.fill('قديم معلق');await page.goBack();await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).waitFor();assert.equal(await search.inputValue(),'أحمد');await page.goForward();await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();assert.equal(await search.inputValue(),'مفقود');
    await search.fill('أحمد');await page.waitForTimeout(350);await page.evaluate(id=>history.pushState(null,'',`/patient-cards?facility_id=${id}`),f.second);await page.getByText('0 بطاقة مريض مطابقة',{exact:true}).waitFor();await page.waitForTimeout(1500);assert.equal(await page.getByText('1 بطاقة مريض مطابقة',{exact:true}).count(),0);
    await page.goto(`${base}/patient-cards?facility_id=${f.other}`);await page.getByRole('heading',{name:'بطاقات المرضى غير متاحة',exact:true}).waitFor();assert.equal(await page.getByRole('region',{name:'جدول بطاقات المرضى'}).count(),0);
  }finally{await context.close();}}
  if(process.env.DOSSIER_SKIP_GALLERY !== '1') await writeFile(new URL('README.md',gallery),'# مراجعة بطاقات المرضى — المرحلة الأولى\n\nلقطات فعلية ببيانات اصطناعية من Chromium، Next standalone → Laravel → MariaDB 10.11.\nالتحميل/الفشل يؤخّران تسليم استجابة Laravel الفعلية أو يفشلان النقل بعدها؛ لا page.route ولا API محاكٍ.\nيستخدم الجدول التمرير الأفقي الداخلي المشترك على الهاتف؛ صور list-actions تظهر الأعمدة والإجراءات بعد التمرير.\n\n'+[390,768,1440].map(w=>`## ${w}px\n\n`+['list','list-actions','oncology-detail','draft-detail','previous-visit','empty','no-visits','loading','error'].map(s=>`- [${s}](${s}-${w}.jpg)`).join('\n')).join('\n\n')+'\n\n![قائمة بطاقات المرضى على سطح المكتب](list-1440.jpg)\n');
});
