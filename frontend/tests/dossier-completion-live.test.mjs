...
function fixture(mode){const r=spawnSync('php',['tests/Support/dossier-completion-live.php',mode],{cwd:fileURLToPath(new URL('../../backend/',import.meta.url)),env:process.env,encoding:'utf8'});assert.equal(r.status,0,r.stdout+r.stderr);assert.doesNotMatch(r.stdout+r.stderr,/In .+ line|Exception/);process.stdout.write(r.stdout);}
before(async()=>{fixture('prepare');f=JSON.parse(readFileSync(new URL('../../backend/storage/framework/testing/dossier-completion-live.json',import.meta.url)));browser=await chromium.launch(process.env.PLAYWRIGHT_CHANNEL?{channel:process.env.PLAYWRIGHT_CHANNEL}:{});await mkdir(gallery,{recursive:true});});
...
async function choose(page,label,search,text=search){await page.getByRole('button',{name:`اختيار: ${label}`,exact:true}).click();const g=page.getByRole('group',{name:label,exact:true});await g.getByRole('searchbox').fill(search);await g.getByRole('button',{name:new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'))}).first().click();}
...
test('resume, multiple clinical entries, prescription, outgoing referral, upload retry and explicit finalization at all widths',async()=>{
  for(const width of [390,768,1440]){
    ...
    try{
      ...
      await page.getByRole('heading',{name:'المرفقات ثم المراجعة',exact:true}).waitFor();
      ...
      await page.getByRole('button',{name:'الانتقال إلى المراجعة',exact:true}).click();await capture(page,`review-${width}`);await page.locator('[name="confirmed"]').check();d=await save(page,'/review','حفظ المراجعة كمسودة');
...
test('later visit reuses visit-only steps, resumes independently and appears above the earlier complete visit',async()=>{
  ...
  try{
    ...
    assert.equal(await page.getByRole('navigation',{name:'مراحل مساحة العمل'}).getByRole('button',{name:/البيانات الشخصية/}).count(),0);
    await page.locator('[name="visit_date"]').fill('2002-02-01');await page.locator('[name="is_referred"]').selectOption('no');
...
test('browser Back cancellation retains the entire unsaved clinical draft',async()=>{
 ...
 test('delayed real clinical doctor responses cannot revive a previous clinic or facility',async()=>{
   ...
   try{
     ...
     await page.getByRole('button',{name:'اختيار: الطبيب المسؤول · الخدمة 1',exact:true}).click();const picker=page.getByRole('group',{name:'الطبيب المسؤول · الخدمة 1',exact:true});await picker.getByRole('button',{name:/الطبيب المسؤول 2/}).waitFor();assert.equal(await picker.getByRole('button',{name:/الطبيب المسؤول 1/}).count(),0);
