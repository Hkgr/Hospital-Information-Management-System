...
async function capture(page,name){await page.evaluate(async()=>{await document.fonts.ready;window.scrollTo({top:0,left:0,behavior:'instant'});await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false,name);assert.equal(await page.evaluate(()=>{const c=document.querySelector('#main-content > div');return c.scrollWidth>c.clientWidth+1;}),false,name+' inner canvas overflow');await page.screenshot({path:fileURLToPath(new URL(name+'.jpg',gallery)),type:'jpeg',quality:85,fullPage:true});await page.screenshot({path:fileURLToPath(new URL(name+'-viewport.jpg',gallery)),type:'jpeg',quality:85});}
