...
test('real three-section wizard, saved/resumed draft, diagnoses, validation and accessible gallery at every width', async () => {
  for (const width of [390, 768, 1440]) {
    const { page, context } = await pageAt(width);
    try {
      await page.getByRole('radio', { name: 'تسجيل مريض جديد', exact: true }).check();
      await page.locator('[name="first_name"]').fill('ليلى'); await page.locator('[name="family_name"]').fill(`اختبار المعالج ${width}`);
      await page.getByRole('radio', { name: 'اختيار مريض موجود', exact: true }).check();
      await choose(page, 'المريض الموجود', f.search_patient_name, f.search_patient_code);
      await page.getByRole('link', { name: 'فتح الإضبارة الموجودة', exact: true }).waitFor(); await capture(page, `existing-patient-${width}`);
      await page.getByRole('radio', { name: 'تسجيل مريض جديد', exact: true }).check(); assert.equal(await page.locator('[name="first_name"]').inputValue(), 'ليلى');
...
test('delayed old doctor response cannot replace a newer clinic or a changed facility', async () => {
  ...
  try {
    ...
    await page.goto(`${base}/dossiers/${id}/edit?facility_id=${f.other}`); await page.getByRole('heading', { name: 'تعذّر فتح الإضبارة', exact: true }).waitFor(); assert.equal(await page.locator('[name="referral_reason"]').count(), 0);
  } finally { await context.close(); }
});
